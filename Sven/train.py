...
if __name__ == "__main__":
    ...
    experiment = wandb.init(project='BDCN', name='test1', entity="michelleappel", id='test')
    ...
    vdataset = ImageDataset(r'C:\Users\appel\Documents\Project\simulation-synthesis\output\448\labeled_fake', augment=True)
    vtrain = Subset(vdataset, range(0, int(0.9*len(vdataset))))
    vval = Subset(vdataset, range(int(0.9*len(vdataset)), len(vdataset)))

    vtrain_loader = DataLoader(vtrain, **params, shuffle=False)
    vval_loader = DataLoader(vval, **params, shuffle=False)

    rdataset = ImageDataset(r'C:\Users\appel\Documents\Project\simulation-synthesis\output\448\labeled_real', augment=True)
    rtrain = Subset(rdataset, range(0, int(0.9*len(rdataset))))
    rval = Subset(rdataset, range(int(0.9*len(rdataset)), len(rdataset)))

    rtrain_loader = DataLoader(rtrain, **params, shuffle=False)
    ...
    print("Training...")
    for step in tqdm(range(start_step, args.max_iter + 1)):
        optimizer.zero_grad()
        batch_loss = 0
        ...
        if vval_loader is not None and step % args.val_step_size == 0:
            ...
            experiment.log({
                ...
                'masks': {
                    'true': wandb.Image(val_labels[0].cpu()),
                    'pred': torch.softmax(val_out[0], dim=0)[1].float().cpu()
