...
def test(model, args):
    ...
    for i, (ms_data, label) in enumerate(testloader):
        ...
        for data in ms_data:
            ...
            ms_fuse += fuse
        ms_fuse /= len(ms_data)
        if not os.path.exists(os.path.join(save_dir, 'fuse_ms')):
            os.mkdir(os.path.join(save_dir, 'fuse_ms'))
        cv2.imwrite(os.path.join(save_dir, 'fuse_ms', '%s.jpg'%i), 255-ms_fuse*255)
...
def parse_args():
    ...
    parser.add_argument('-m', '--model', type=str, default='.\params\bdcn_pretrained_on_nyudv2_rgb.pth',
