...
def test(model, args):
    ...
    if save_sideouts:
        for j in range(5):
            os.makedirs(os.path.join(save_dir, 's2d_'+str(j)))
            os.makedirs(os.path.join(save_dir, 'd2s_'+str(j)))
    ...
    all_t = 0
    for nm in test_lst:
        data = cv2.imread(os.path.join(test_root, nm))
        ...
        #         cv2.imwrite(os.path.join(save_dir, 's2d_'+str(k), '%s.jpg'%i), 255-t*255)
        #         # savemat(osp.join(save_dir, 'd2s_'+str(k), nm+'.mat'), {'prob': out[j+5]})
        #         cv2.imwrite(os.path.join(save_dir, 'd2s_'+str(k), '%s.jpg'%i), 255-255*t)
        #         k += 1
        # else:
        out = [F.sigmoid(out[-1]).cpu().data.numpy()[0, 0, :, :]]
        if not os.path.exists(os.path.join(save_dir, 'fuse_image')):
            os.mkdir(os.path.join(save_dir, 'fuse_image'))
        cv2.imwrite(os.path.join(save_dir, 'fuse_image/%s.png'%nm.split('/')[-1].split('.')[0]), 255*out[-1])
        all_t += time.time() - t1
    print(all_t)
