...
def test(model, args):
    ...
    test_img = Data(test_root, None, mean_bgr=mean_bgr)
    ...
    all_t = 0
    for i, (data, _) in tqdm(enumerate(testloader)):
        if args.cuda:
            data = data.cuda()
        ...
        fuse = out[-1].cpu().data.numpy()[0, 0]
        if not os.path.exists(os.path.join(save_dir, 'fuse')):
            os.mkdir(os.path.join(save_dir, 'fuse'))
        cv2.imwrite(os.path.join(save_dir, 'fuse', '%s.png'%i), 255-fuse*255)
        all_t += time.time() - tm
    print(all_t)
