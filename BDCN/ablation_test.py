...
def test(model, args):
    ...
    for i, (data, _) in enumerate(testloader):
        ...
        if not os.path.exists(os.path.join(save_dir, 'fuse_ablation')):
            os.mkdir(os.path.join(save_dir, 'fuse_ablation'))
        cv2.imwrite(os.path.join(save_dir, 'fuse_ablation', '%s.jpg'%i), 255-t*255)
        all_t += time.time() - t1
