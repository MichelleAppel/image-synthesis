...
def train(model, args):
    experiment = wandb.init(project='BDCN', name=args.dataset, entity="michelleappel")
    experiment.config.update(dict(batch_size=args.batch_size))
    ...
    train_img = Data(data_root, None, yita, mean_bgr=mean_bgr, crop_size=crop_size, do_domain_transfer=args.domain_transfer)
    ...
    for step in tqdm(range(start_step, args.max_iter + 1)):
        ...
        if step % args.display == 0:
            ...
            # wandb log
            experiment.log({
                'images': wandb.Image(images[0].detach().flip(0).cpu()), #BGR>RGB
                'masks': {
                    'true': wandb.Image(labels[0].detach().cpu()),
                    'pred': wandb.Image(out[-1][0].detach().squeeze(0).squeeze(0).float().cpu())
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', type=str, choices=cfg.config.keys(),
        default='NYUDv2', help='The dataset to train')
    ...
    parser.add_argument('--domain_transfer', action='store_true',
        help='domain transfer on the input from fake to real')
